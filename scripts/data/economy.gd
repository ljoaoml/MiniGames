...
signal animal_inventory_changed(animal_id: String, count: int)
signal selected_crop_changed(crop_id: String)
signal selected_tree_changed(tree_id: String)
signal selected_animal_changed(animal_id: String)
...
var selected_animal: String = "galinha"
var selected_tool: String = "none" # "none", "hoe", "plant", "plant_tree", "plant_animal", "remove"
...
func buy_animals(animal_id: String, quantity: int) -> bool:
	var data: Dictionary = Animals.ANIMALS[animal_id]
	var total_cost: int = data.cost * quantity
	if coins < total_cost:
		return false
	coins -= total_cost
	animal_inventory[animal_id] += quantity
	coins_changed.emit(coins)
	animal_inventory_changed.emit(animal_id, animal_inventory[animal_id])
	select_animal(animal_id)
	return true

func consume_animal(animal_id: String) -> void:
	animal_inventory[animal_id] -= 1
	animal_inventory_changed.emit(animal_id, animal_inventory[animal_id])

func select_animal(animal_id: String) -> void:
	selected_animal = animal_id
	selected_animal_changed.emit(animal_id)
	select_tool("plant_animal")
